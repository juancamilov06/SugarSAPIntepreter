...
class inmuebles_methods{	
    ...
    function afterSave($bean, $event, $arguments)
    {
		...
		if(isset($arguments['isUpdate']) && $arguments['isUpdate'] == false){			
			...
			$request = new Request();
			$request = $request->setPostMethod()->setHeaders($this->headers)->setInformation($info)->setUrl("http://localhost:4567/api/inmueble");        
			...
			$info = $this->mapBean($bean);      
			$request = new Request();
			$request = $request->setPUTMethod()->setHeaders($this->headers)->setInformation($info)->setUrl("http://localhost:4567/api/inmueble");        
